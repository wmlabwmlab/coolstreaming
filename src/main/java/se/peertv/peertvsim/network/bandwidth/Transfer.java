...
public class Transfer {
	...
	public Transfer(double bandwidthInBPS, BandwidthManager sourceBandwidthManager,
		...
		this.originalStartTime = this.startTime = scheduler.now;
	...
	public long getRemainingBytes() throws Exception
	{
		...
		if(totalBytes <= bytesPerMilliSec)
			return 0;
//		
		long timeInQueue = scheduler.now - startTime;
	...
	public void resetBandwidth(double newBandwidth) throws Exception {		
		...
		if(deliveryEvent.time < scheduler.now)
		{
			System.err.println("dlvr: " + deliveryEvent.time + " < " + " now: " +scheduler.now);
			A.ssert(deliveryEvent.time >= scheduler.now);			
		}
		
		if(this.deliveryEvent.time == scheduler.now)
		{
		...
		{	
			...
			long oldTimeRemaining = deliveryEvent.time - Scheduler.getInstance().now; 
			...
			if(oldTimeRemaining > 0)
			{
				...
				deliveryEvent.setTime( newTime + scheduler.now );
